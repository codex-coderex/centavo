...
def create_budget(user_id: int, name: str, period_type: str, start_date, end_date=None):
    name = name.strip()
    ...
    period_type = normalize_enum_value(period_type, BudgetPeriod, "Invalid budget period")
    start_date = require_datetime(start_date, "Start date")
    end_date = parse_datetime(end_date, "End date")
    if end_date is not None and end_date < start_date:
        raise ValueError("End date cannot be before start date")

    budget_id = budgets_q.create_budget(user_id=user_id, name=name, period_type=period_type, start_date=start_date, end_date=end_date)
    return {"budget_id": budget_id}


def update_budget(budget_id: int, name: str | None = None, period_type: str | None = None, start_date=None, end_date=None):
    ...
    if period_type is not None:
        kwargs["period_type"] = normalize_enum_value(period_type, BudgetPeriod, "Invalid budget period")
    if start_date is not None:
        kwargs["start_date"] = require_datetime(start_date, "Start date")
    if end_date is not None:
        kwargs["end_date"] = parse_datetime(end_date, "End date")

    next_start = kwargs.get("start_date", budget["start_date"])
    next_end = kwargs.get("end_date", budget["end_date"])
...
def create_budget_item(budget_id: int, category_id: int, planned_amount, rollover_enabled: bool = False):
    budget = budgets_q.get_budget(budget_id)
    if budget is None:
        raise ValueError("Budget does not exist")
    ...
    budget_item_id = budgets_q.create_budget_item(budget_id=budget_id, category_id=category_id, planned_amount_minor=planned_amount_minor, rollover_enabled=rollover_enabled)
    return {"budget_item_id": budget_item_id}


def update_budget_item(budget_item_id: int, planned_amount=None, category_id: int | None = None, rollover_enabled: bool | None = None):
