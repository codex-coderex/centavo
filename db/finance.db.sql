...
CREATE TABLE IF NOT EXISTS "budget_item" (
	...
	PRIMARY KEY("budget_item_id" AUTOINCREMENT),
...
CREATE TABLE IF NOT EXISTS "recurring_rule" (
	...
	"end_date"	TEXT,
	"status"	TEXT NOT NULL DEFAULT 'active',
	PRIMARY KEY("recurring_rule_id" AUTOINCREMENT),
...
CREATE INDEX IF NOT EXISTS "ix_budget_user_start_date" ON "budget" (
	"user_id",
	"start_date"
);
...
CREATE INDEX IF NOT EXISTS "ix_recurring_rule_status_next_due_date" ON "recurring_rule" (
	"status",
	"next_due_date"
);
...
CREATE INDEX IF NOT EXISTS "ix_transaction_account_date" ON "transaction" (
	"account_id",
	"transaction_date"
);
...
CREATE UNIQUE INDEX IF NOT EXISTS "uq_account_active_user_name" ON "account" (
	"user_id",
	"name"
) WHERE "status" = 'active';
COMMIT;
